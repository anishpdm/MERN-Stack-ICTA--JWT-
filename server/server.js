...
mongoose.connect("mongodb+srv://anish:anish@cluster0.c6gnj.mongodb.net/myFirstDatabase?retryWrites=true&w=majority");
var fs = require('fs');
...
app.post('/test',(req,res)=>{
  jwt.verify(req.headers.token, 'ictacademy', function (err, decoded) {
    if (decoded && decoded.user) {
    ...
    if (req.body && req.body.username && req.body.password) {
      user.find({ username: req.body.username }, (err, data) => {
        if (data.length > 0) {
          ...
          // console.log(bcrypt.compare(req.body.password,data[0].password))

          const validPassword =  bcrypt.compare(req.body.password, data[0].password);
          if (validPassword) {
            checkUserAndGenerateToken(data[0], req, res);
          } else {

            res.status(400).json({
              errorMessage: 'Username or password is incorrect!. Validation failed',
              status: false
            });
          }
...
function checkUserAndGenerateToken(data, req, res) {
  ...
  jwt.sign({ user: data.username, id: data._id },
     'ictacademy', { expiresIn: '1d' }, (err, token) => {
    if (err) {
      res.status(400).json({
        status: false,
        errorMessage: err,
